...
logger = logging.getLogger('spareparts-scraper.piecesauto24')

def scrape(search_terms=None, max_pages=3):
    """
    Scrape les données de piecesauto24.com
    ...
    if not search_terms:
        search_terms = ["filtre", "plaquette", "frein", "huile", "batterie"]
    ...
    for term in search_terms:
        ...
        for page in range(1, max_pages + 1):
            try:
                url = f"https://www.piecesauto24.com/product-search/1/{term}/?page={page}"
                ...
                for product in products:
                    try:
                        ...
                        reference_elem = product.select_one('.product-number')
                        if reference_elem:
                            item['reference'] = reference_elem.text.strip().replace("N° d'article: ", "")
                        else:
                        ...
                        # URL du produit
                        url_elem = product.select_one('.product-title a')
                        item['url'] = "https://www.piecesauto24.com" + url_elem['href'] if url_elem and 'href' in url_elem.attrs else None
                        
                        # Image
                        img_elem = product.select_one('.product-image img')
                        ...
                        # Disponibilité
                        stock_elem = product.select_one('.product-availability')
                        if stock_elem:
                            stock_text = stock_elem.text.strip().lower()
                            item['in_stock'] = "en stock" in stock_text or "disponible" in stock_text
    ...
    logger.info(f"Scraping terminé pour PiecesAuto24. Total: {len(results)} produits")
    return results
